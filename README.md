...
This repo implements the trustless EVM multichain bridge (now termed "ExoBridge") contract. For more background on the concept, see [this article]([https://docs.exohood.com/how-to-guides/multichain-bridge). 

# Bridge Basics

A bridge exists as multichain contract (`Bridge.sol`) on two separate EVM-based blockchains. A set of participants may stake (`Bridge.stake()`) a specified token (`Bridge.stakeToken()`) to enter the pool of proposer candidates. Every time a piece of data is submitted to a bridge, a new proposer is chosen pseudorandomly (`Bridge.getProposer()`) with probability proportional to the participant's stake.
