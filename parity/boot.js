// Boot a parity MuPoS (single node) with one or more specified ports
...
// Create a directory for the MuPoS data if it doesn't exist
        ...
        setTimeout(() => {
          ...
          jsonfile.readFile(`${PATH}/config.json`, (err, file) => {
            ...
            jsonfile.writeFile(`${PATH}/config.json`, config, { spaces: 2}, () => {
              ...
              // https://github.com/exohood/exobridge/
              // args.push('--no-ipc'); 
              ...
              console.log(`${new Date()} Parity MuPoS #${i} started. RPC port=${port} WS port=${wsPort}`);
            })
          });
        }, 500)
